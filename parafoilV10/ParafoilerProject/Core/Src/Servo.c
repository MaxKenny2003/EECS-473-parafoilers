...
void turnLeft(float angle) {
	...
	float inc = 85.0/180.0;
	float c_speed = 274 - inc*angle;
	float  cc_speed = 294 + inc*angle;
	spin_servo((int)c_speed, SERVO_L);
	spin_servo((int)cc_speed, SERVO_R);
}
...
void turnRight(float angle) {
	...
	volatile float c_speed = 274 - inc*angle;
	volatile float  cc_speed = 294 + inc*angle;

	spin_servo((int)cc_speed, SERVO_L);
	spin_servo((int)c_speed, SERVO_R);
}
