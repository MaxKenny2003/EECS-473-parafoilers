    ...
    for(;;)
    {
        if (osSemaphoreAcquire(gpsDataReadySemHandle, osWaitForever) == osOK)
        {
            ...
            if(myGpsData.newData) {
                ...
                target_angle = calc_angle(targetLocation.longitude, targetLocation.latitude, myGpsData.Longitude, myGpsData.Latitude);
                osMutexAcquire(printfMutexHandle, osWaitForever);
                printf("Angle to turn: %.6f\r\n", target_angle);
                osMutexRelease(printfMutexHandle);
            }
        }
    }
  ...
  for(;;)
  {
	    ...
	    if(control_mode == MODE_MANUAL_FALLBACK || control_mode == MODE_MANUAL_OVERRIDE) {
	        ...
	        if(manual_servo_command == 1) {
	            spin_servo(345, SERVO_L);  // Left
	        } else if(manual_servo_command == 2) {
	            spin_servo(230, SERVO_R);  // Right
	        } else {
	            stop_servo(SERVO_L);
	        }
    }
        ...
        {
        	float turn_angle = fmod(target_angle - heading_real, 180.0);

            osMutexAcquire(printfMutexHandle, osWaitForever);
            printf("Servo Turn Angle: %f\r\n", turn_angle);
            osMutexRelease(printfMutexHandle);
...
//  {
//	  taskENTER_CRITICAL();
//	  spin_servo_clockwise(SERVO_R);
//	  spin_servo_counterclockwise(SERVO_L);
//	  taskEXIT_CRITICAL();
//
//	  vTaskDelayUntil(&xLastWakeTime, xFrequency);
//
//	  taskENTER_CRITICAL();
//	  spin_servo_counterclockwise(SERVO_R);
//	  spin_servo_clockwise(SERVO_L);
//	  taskEXIT_CRITICAL();
//
//	  vTaskDelayUntil(&xLastWakeTime, xFrequency);
//  }
  ...
  for(;;)
  {
    ...
    if(osSemaphoreAcquire(imuDataReadySemHandle, 100) == osOK) {
        ...
        if(data_available(&sensor1)) {
            ...
            float yaw_rad = atan2f(2.0f * (quat_real * quat_k + quat_i * quat_j),
                                   1.0f - 2.0f * (quat_j * quat_j + quat_k * quat_k));
            float imu_heading = yaw_rad * 180.0f / 3.14159265f;  // Convert to degrees
            if (imu_heading < 0) imu_heading += 360.0f;  // Normalize to 0-360
            imu_heading = 360.0f - imu_heading;

            if(myGpsData.headMot != -1) {
            	heading_offset = myGpsData.headMot - imu_heading;
            }

            heading_real = fmod(imu_heading + heading_offset, 360.0);
            ...
            if (current_time - last_print_time >= 500) {
                if(osMutexAcquire(printfMutexHandle, 10) == osOK) {
                    printf("Hreal[%.3f] H[%.3f] Off[%.3f] Q[%.3f,%.3f,%.3f,%.3f] A[%.2f,%.2f,%.2f] G[%.3f,%.3f,%.3f] %s\r\n",
                           heading_real, imu_heading, heading_offset, quat_i, quat_j, quat_k, quat_real,
