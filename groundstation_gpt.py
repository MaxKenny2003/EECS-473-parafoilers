...
class XBeeDashboard(tk.Tk):
    ...
    def _rxSensorData(self):
        ...
        payload = struct.unpack('<12f', rxData[0:expected_len-1])
        self.telemetryGPSLatVar.set(round(payload[0], 6))
        self.telemetryGPSLonVar.set(round(payload[1], 6))
        self.telemetryGPSAltVar.set(round(payload[2], 2))

        self.telemetryAccelXVar.set(round(payload[3], 3))
        self.telemetryAccelYVar.set(round(payload[4], 3))
        self.telemetryAccelZVar.set(round(payload[5], 3))

        self.telemetryGyroRollVar.set(round(payload[6], 3))  
        self.telemetryGyroPitchVar.set(round(payload[7], 3)) 
        self.telemetryGyroYawVar.set(round(payload[8], 3))   

        self.telemetryEulerIVar.set(round(payload[9], 3))   
        self.telemetryEulerJVar.set(round(payload[10], 3))  
        self.telemetryEulerKVar.set(round(payload[11], 3))
        self._log(f"RX: GPS(Lat={payload[0]:.3f}, Lon={payload[1]:.3f})")
    ...
    def _periodic_ui_update(self):
        ...
        if self.serial_port and self.serial_port.is_open:
            try:
                ...
                while self.serial_port.in_waiting >= 49: # 49 bytes = 1 full packet
                    self._rxSensorData()
            except Exception as e:
