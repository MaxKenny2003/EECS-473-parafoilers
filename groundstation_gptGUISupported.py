...
try:
    while True:
        cmd = input("Enter CMD: ")
        # Todo: Ask Matteo (and team) if we should keep this current method of sending packets behind the scenes (have a button to specifically send packets)
        # or if I should make buttons for every option instead
        if cmd == "exit":
